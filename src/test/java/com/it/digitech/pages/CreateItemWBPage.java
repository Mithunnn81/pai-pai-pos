package com.it.digitech.pages;
	...
	public class CreateItemWBPage extends CommonMethods{

		ExtentTest test;
		
		public CreateItemWBPage (ExtentTest test) {
		...
		@FindBy (xpath= "//a[@class='dropdown-item'][normalize-space()='Create Item']")
...
//		private WebElement lotControled;
		...
		public void CreateItemWB() throws IOException {
			
			clickAndCapture(test, base, "base");
	        clickAndCapture(test, item, "item");
	        clickAndCapture(test, createItem, "createItem");
	        ...
	        String title = getUniqueTitle("Item");
	        typeAndCapture (test, itemName, "itemName", title );      
	        clickAndCapture(test, itemCatagory, "itemCatagory");
	        clickAndCapture(test, itemCatagorySelect, "itemCatagorySelect");
//	        clickAndCapture(test, serialContorled, "serialContorled");
	        clickAndCapture(test, purchaseTab, "purchaseTab");
	        selectFromDropdown(test, purchaseUoM, "purchaseUoM", 24);
	        double randomPrice = getRandomDecimal(200, 300);
	        typeAndCapture (test, purchasePrice, "purchasePrice", String.valueOf(randomPrice) );        
